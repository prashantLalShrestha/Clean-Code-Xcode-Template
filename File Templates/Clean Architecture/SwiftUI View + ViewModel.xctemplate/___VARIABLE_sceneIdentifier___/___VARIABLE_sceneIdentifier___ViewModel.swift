...
class ___VARIABLE_sceneIdentifier___ViewModel: ViewModel {
    ...
    init(flow: ___VARIABLE_sceneIdentifier___ViewFlowClosure) {
        ...
        input.flatMap { [unowned self] in self.mapToResult($0) }
            .compactMap { $0 }
            .sink { [unowned self] in self.reduce($0) }
            .store(in: &bag)
...
private extension ___VARIABLE_sceneIdentifier___ViewModel {
    ...
    func mapToResult(_ input: ___VARIABLE_sceneIdentifier___ViewInput) -> AnyPublisher<___VARIABLE_sceneIdentifier___ViewResult?, Never> {
        switch input {
        case .onAppear:
...
private extension ___VARIABLE_sceneIdentifier___ViewModel {
    ...
    func reduce(_ result: ___VARIABLE_sceneIdentifier___ViewResult) { }
}
